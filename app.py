...
# @app.route('/api/survey/<int:survey_id>/participation', methods=['GET'])
# def get_survey_participation(survey_id):
#     try:
#         conn, cursor = get_db()
#         cursor.execute("SELECT COUNT(*) FROM SurveyAssignments WHERE SurveyID = ?", (survey_id,))
...
#         not_submitted = total - submitted if total > 0 else 0
...
#     except Exception as e:
#         print("Participation API error:", str(e))
#         return jsonify({"status": "error", "message": str(e)}), 500
...
def get_combined_participation():
    try:
        ...
        not_submitted = total_assigned - total_submitted if total_assigned > total_submitted else 0
...
@app.route('/api/employees/assigned-vs-attended', methods=['GET'])
def get_assigned_vs_attended():
    try:
        ...
        print("Error in get_assigned_vs_attended:", e)
        return jsonify({"status": "error", "message": str(e)}), 500
